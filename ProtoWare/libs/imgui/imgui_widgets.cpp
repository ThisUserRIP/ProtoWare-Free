...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    struct button_animation { ImColor frame; };
    ...
    tab_map_item->second.frame = ImLerp(
        tab_map_item->second.frame.Value,
        pressed || held ? (ImVec4)(ImColor)GetColorU32(ImGuiCol_FrameBgActive) :
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, IM_COL32(25,25,25,255), 2.50f);
    window->DrawList->AddRect(bb.Min, bb.Max, IM_COL32(30, 30, 30, 255), 2.50f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, IM_COL32(213, 115, 115, 25), style.ScrollbarRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f - 5.0f));
    ...
    if (tab_map_item == tab_map.end())
    {
        ...
        tab_map_item->second.opacity = 0.0f;
        //tab_map_item->second.frame = GetColorU32(ImGuiCol_FrameBg);
    }

    tab_map_item->second.opacity = ImLerp(tab_map_item->second.opacity, *v ? 1.0f : 0.0f, std::min(1.0f, g.IO.DeltaTime * 8.f));
    ...
    //RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBg), false, 0.f);
    //RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBg), false, 0.f);
    //RenderFrame(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), GetColorU32(tab_map_item->second.frame.Value), false, 0.f);
    
    // toggle
    window->DrawList->AddRect(ImVec2(check_bb.Min.x + 2, check_bb.Min.y + 2), ImVec2(check_bb.Max.x - 2, check_bb.Max.y - 2), IM_COL32(159, 89, 89, 255 * tab_map_item->second.opacity), 1.0f);
    window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 3, check_bb.Min.y + 3), ImVec2(check_bb.Max.x - 3, check_bb.Max.y - 3), IM_COL32(213, 115, 115, 255 * tab_map_item->second.opacity));

    // border
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, IM_COL32(213, 115, 115, 255), 2.5f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (tab_map_item == tab_map.end())
    {
        ...
        //tab_map_item->second.frame = (ImColor)GetColorU32(ImGuiCol_FrameBg);
        tab_map_item->second.frame = IM_COL32(28, 28, 28, 255);
    }
    ...
    tab_map_item->second.frame = ImLerp(tab_map_item->second.frame.Value, hovered ? (ImVec4)(ImColor)IM_COL32(35, 35, 35, 255) : (ImVec4)(ImColor)IM_COL32(28, 28, 28, 255), std::min(1.0f, g.IO.DeltaTime * 8.f));
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRect(frame.Min, frame.Max, IM_COL32(30,30,30,255), 1.5f);
        window->DrawList->AddRectFilled(ImVec2(frame.Min.x + 1, frame.Min.y + 1), ImVec2(frame.Max.x - 1, frame.Max.y - 1), tab_map_item->second.frame);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (label_size.x > 0.0f)
    {
        ...
        ImVec2 textValSize = CalcTextSize(value_buf);
        ImVec2 tetValPos = ImVec2(total_bb.Max.x - textValSize.x, total_bb.Min.y);
        RenderFrame(ImVec2(tetValPos.x - 4, tetValPos.y - 1), ImVec2(total_bb.Max.x + 3, tetValPos.y + textValSize.y + 1), IM_COL32(28, 28, 28, 255), true, 1.5f);
        RenderText(tetValPos, value_buf);
    }
    ...
    const bool value_changed = SliderBehavior(ImRect(ImVec2(total_bb.Min.x, total_bb.Min.y + label_size.y + 7.f), ImVec2(total_bb.Max.x, total_bb.Min.y + label_size.y + 12.f)), id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    // background
    RenderFrame(ImVec2(total_bb.Min.x + 1, total_bb.Min.y + label_size.y + 7.f), ImVec2(total_bb.Max.x - 1, total_bb.Min.y + label_size.y + 11.f), IM_COL32(26, 26, 26, 255), true, 0.f);
    RenderFrame(ImVec2(total_bb.Min.x, total_bb.Min.y + label_size.y + 8.f), ImVec2(total_bb.Max.x, total_bb.Min.y + label_size.y + 11.f), IM_COL32(28,28,28,255), true, 0.f);
    RenderFrame(ImVec2(total_bb.Min.x + 1, total_bb.Min.y + label_size.y + 11.f), ImVec2(total_bb.Max.x - 1, total_bb.Min.y + label_size.y + 12.f), IM_COL32(26, 26, 26, 255), true, 0.f);


    // slider
    RenderFrame(ImVec2(total_bb.Min.x + 1, total_bb.Min.y + label_size.y + 7.f), ImVec2(tab_map_item->second.thumb_position - 1, total_bb.Min.y + label_size.y + 8.f), IM_COL32(186, 102, 102, 255), true, 0.f);
    RenderFrame(ImVec2(total_bb.Min.x, total_bb.Min.y + label_size.y + 8.f), ImVec2(tab_map_item->second.thumb_position, total_bb.Min.y + label_size.y + 11.f), IM_COL32(240, 128, 128, 255), true, 0.f);
    RenderFrame(ImVec2(total_bb.Min.x + 1, total_bb.Min.y + label_size.y + 11.f), ImVec2(tab_map_item->second.thumb_position - 1, total_bb.Min.y + label_size.y + 12.f), IM_COL32(186, 102, 102, 255), true, 0.f);
